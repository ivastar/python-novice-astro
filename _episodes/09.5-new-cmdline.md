...
The ipython interpreter, Jupyter Notebook and other interactive tools are great for prototyping code and exploring data,
...
save the following in a text file called `argv_list.py`.
...
With this in hand, let's build a version of out temperature conversion 
program.
The first step is to write a function that does our implementation,
...
and the value to process from `sys.argv[1]`.
Here's a simple test:

~~~
$ python temperature.py 100
~~~
{: .language-bash}
...
> of that file (e.g., when importing `temperature.py`,
> `__name__` is `'temperature'`). However, when running a
...
The next step is to teach our program how to handle multiple inputs.
